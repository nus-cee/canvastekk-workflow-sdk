...
- [x] **2.1** `definition.ts`: zod schema renames identity `name` → `slug` (keep SLUG_PATTERN refine, updated messages) and display `title` → `name` (required); implement the same Vocabulary contract rule as a pre-transform (STRICT legacy trigger: no `slug` + `name` + `title` → map + `console.warn`; no `slug ?? name` fallback; `name`-only fails with the disambiguating message; mixed slug+title warns + ignores); `getNodeId` picks `"slug" | "version"`; inbound `id` stripping stays
    ...
    — **Done:** schema object renamed to internal `WorkflowNodeManifestObjectSchema` with flipped fields; `WorkflowNodeManifestSchema = z.preprocess(mapLegacyVocabulary, …)` implements all four rules with console warnings; `getNodeId` reads slug; id-stripping transform unchanged; files: `typescript/src/definition.ts`; fixes: object-close syntax while wiring the preprocess
    — **Consumers affected:** ts SDK consumers, future register CLI (ts flavor)

- [x] **2.2** Sweep ts internal identity reads: `base-node.ts`, `registry.ts`, `app.ts`, `logging.ts`, `workflow/builder.ts`, `middleware.ts` — rename ONLY node-identity reads (domain `name` fields in `contracts/` are NOT node identity; leave them); **do not touch `workflow/models.ts`** (already canonical engine wire); update ts tests incl. manifest shape + the compat cases from 2.1; run gate `tsc --noEmit` + `vitest run` + `tsup` green from `typescript/`
    ...
    — **Done:** base-node.ts 3 identity reads (deprecation guard + both log `nodeName` values), registry.ts buildRegistryPayload (`name`=slug value, `label`=display — engine register shape), diff.ts HANDLED_KEYS + slug-mismatch reads/message (parity with python, missed in the plan's file list); `workflow/models.ts` untouched (0-line diff); generic dict helpers (`extractNodeData`, `registerNodeResultGet/Has`) deliberately NOT flipped — passthrough payloads, not manifest identity; new compat describe-block pins all five determinism rules; gate green: 309 tests, tsc clean, tsup build success; files: 4 src + 6 test files; fixes: bulk-regex over-reach on generic dicts reverted, invalid-slug test override key, def.title assertion
...
- [x] **3.1** Sync docs per AGENTS.md: `docs/EXTERNAL-AUTHOR-GUIDE.md`, `python/README.md`, `typescript/README.md`, root `README.md` (manifest field tables/examples), `python/.../data/skills/canvastekk-node-builder/SKILL.md` + `canvastekk-node-patterns/SKILL.md` (they teach `name=`/`title=` construction), and `examples/echo_node/` to the new spellings (keep a short migration note: legacy kwargs still accepted with a warning)
    ...
    — **Done:** 25 constructor snippets flipped across guide/READMEs/skills/example (scripted, lowercase-literal rule); node-builder skill template fixed (removed the manual `id=` line that pre-dated this change and contradicted the guide, de-duplicated the name key, slug assertion); guide Node-ID section re-worded to slug + migration note added; both READMEs' required-field lists + python README field-mapping table updated (slug→engine name, display name→label); root README needed no changes (no construction snippets); files: 6; fixes: duplicate-key template artifact from the scripted flip
    — **Consumers affected:** external node authors, internal skill consumers

- [x] **3.2** Release safety verification + final gates: confirm the merge commit carries `feat!:` + a `BREAKING CHANGE:` footer (per `cliff.toml` `features_always_bump_minor=true` any feat-bearing commit yields ≥0.27.0 — never patch; the `!` + footer additionally flag breaking through the dispatch payload); verify the REAL deploy-safety invariant at merge time — all four consumer repos still pin the SDK by exact wheel URL (nodes ×3 app variants, engine, ff-app, wall-app) and `deploy-lambda.yml` has no version-range/upgrade step; run both packages' full gates one final time from a clean tree
    ...
    — **Done:** pins verified with live evidence — nodes v0.26.1 wheel URL ×3 (fastapi_app, fastapi_app_open3d, fastapi_app_ml), engine v0.25.0, ff-app v0.25.0 (fastapi_app/pyproject.toml), wall-app v0.14.1 (fastapi_app/pyproject.toml, default branch main); nodes deploy-lambda.yml has no pip upgrade/range step; both phase commits carry `feat(definition)!:` + `BREAKING CHANGE:` footers (PR title will carry `feat!:` for the squash); final gates green from clean tree: python 686 passed + ruff clean, ts 309 passed + tsc clean + tsup ok; fixes: none
