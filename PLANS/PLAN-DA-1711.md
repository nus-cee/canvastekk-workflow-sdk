...
- [x] **2.1** Fix START seeding with MERGE semantics: in `runner.py:164-193` and `runner.ts:86-95,112-139`, the level-loop control handler computes `outputs = {**resolved_static, **seeded_run_inputs}` instead of the identity `start_handler` re-run clobbering the seed with `{}` (`_control_flow.py:17-22`, `control-flow.ts:24`). Do NOT simply skip the handler — that would discard static inputs declared on the start node and omit `__start__`'s NodeResult.
    ...
    — **Done:** seeded_outputs dict + `{**resolved, **seeded}` merge before handler exec in both runners (seeded wins, static preserved, __start__ NodeResult recorded); regression tests non-empty inputs run + merge semantics; files: python/canvastekk_workflow_sdk/workflow/runner.py, typescript/src/workflow/runner.ts, python/tests/test_workflow_runner.py, typescript/tests/workflow-runner.test.ts; fixes: none

- [x] **2.2** Wrap per-node `resolve_inputs` in try/except → `NodeResult(status="failed", error=...)` at BOTH resolve sites in each runner — the user-node site (`runner.py:252`, `runner.ts:176`) AND the control-node site (`runner.py:186`, `runner.ts:116`), which is equally outside its try block today.
...
- [x] **2.3** Give each node a per-node subdirectory for outputs/downloads by default (`run_output_dir/<node_id>/`); document that inter-node file hand-off ALREADY works via absolute-path strings passed through edge output values (`test_workflow_runner.py:460-484` FileWriter/Reader) — per-node subdirs preserve this, so no new hand-off machinery (YAGNI). The only breakage is code globbing the `run_output_dir` root (Py `runner.py:152-157`, `context.py:86-111`; TS `runner.ts:91,117,177`, `context.ts:78-84`).
    ...
    — **Done:** ExecutionContext per node gets output_dir=run_output_dir/<node_id>/ (both langs, control+user sites); absolute-path hand-off preserved (FileWriter/Reader tests pass); collision regression test two parallel nodes same filename; files: python/canvastekk_workflow_sdk/workflow/runner.py, typescript/src/workflow/runner.ts, python/tests/test_workflow_runner.py, typescript/tests/workflow-runner.test.ts; fixes: updated 2 pre-existing tests asserting run-dir==node-dir to new subdir contract
...
- [x] **4.1** Upload failure surfacing (`uploads.py:94-98`, `uploads.ts:69-71`): PRIMARY fix = fail the execution on upload failure (`status:"fail"` + `error_code:"UPLOAD_FAILED"` — NO response schema change). Per-field upload status is an OPTIONAL additive field, gated on verifying the engine (canvastekk-workflow-engine, the consumer of the `/execute` wire contract) tolerates unknown response fields — `NodeExecutionResponse` (`response.py:14-49`, `response.ts:3-12`) has no room for it today. Add explicit upload timeout + retry: TS switch `readFileSync` to stream upload (`fs.createReadStream` → fetch body — whole-file RAM read is the same OOM primitive as TS-9 for multi-GB outputs); Py `httpx.put` currently inherits the 5 s default timeout (large uploads likely fail today).
    ...
    — **Done:** PRIMARY fix implemented: upload failure → status:'fail' + error_code:'UPLOAD_FAILED' (NO schema change; Py model_copy, TS object spread); Py httpx.put timeout 600s; TS stream upload via createReadStream + AbortSignal.timeout(600s) replacing readFileSync OOM path; per-field variant NOT taken (engine parser unverified); tests updated (Py raises on upload failure; app-level fail responses both langs); files: python/canvastekk_workflow_sdk/uploads.py, python/canvastekk_workflow_sdk/app.py, typescript/src/uploads.ts, typescript/src/app.ts, python/tests/test_uploads.py; fixes: none

- [x] **4.2** TS resolver hardening (`resolver.ts:67,69,94,126`): reject `__proto__`/`constructor`/`prototype` keys in `to_input` and on merge; use `Object.hasOwn` instead of `in`; merge via create-data-property helper instead of assignment.
    ...
    — **Done:** DANGEROUS_KEYS (__proto__/constructor/prototype) rejected in to_input (throws ResolverError FORBIDDEN_KEY, code union extended) + skipped on merge; Object.hasOwn replaces `in` at both lookup sites; defineOwn (Object.defineProperty data property) replaces assignment; resolver tests 18/18 + new pollution cases via suite; files: typescript/src/workflow/resolver.ts; fixes: none

- [x] **4.3** HttpExecutor: validate remote responses with `NodeExecutionResponseSchema.safeParse` (TS `executor.ts:116-120`, drop bare `as Record` cast); Python: shared `AsyncClient` + bounded response reads + retry/backoff (`executor.py:118-126`); add a small per-level concurrency cap in both runners.
    ...
    — **Done:** TS HttpExecutor safeParses remote JSON via NodeExecutionResponseSchema (malformed → clean error, no bare cast); Py HttpExecutor: shared AsyncClient (connection reuse), retries=2 w/ exp backoff on TransportError/502/503/504, response shape checks (non-dict/non-dict-outputs → RuntimeError); per-level concurrency cap: Py asyncio.Semaphore(8) via _execute_with_sem, TS gatherWithLimit chunked allSettled (both configurable via max_concurrency/maxConcurrency); files: typescript/src/workflow/executor.ts, python/canvastekk_workflow_sdk/workflow/executor.py, python/canvastekk_workflow_sdk/workflow/runner.py, typescript/src/workflow/runner.ts; fixes: none

- [x] **4.4** Python hygiene batch: `re.fullmatch` for slug/semver patterns (`definition.py:25-26` pattern definitions; `.match()` uses at `definition.py:238,247`); fix `extra` log-field merging (`logging.py:85`); sanitize `node_id` in logger names (`logging.py:164-174`); per-execution temp-dir cleanup on the HTTP server path after upload (`context.py:46-54` / `app.py`).
    ...
    — **Done:** re.fullmatch for slug/semver (definition.py); extra={} log fields merged via non-standard record attrs (+ legacy _extra dict honored — pre-existing test contract); get_node_logger sanitizes dots/spaces in node_id; per-execution /tmp/{run}/{node} cleanup after uploads on HTTP path (shutil.rmtree ignore_errors); files: python/canvastekk_workflow_sdk/definition.py, python/canvastekk_workflow_sdk/logging.py, python/canvastekk_workflow_sdk/app.py; fixes: logging merge initially broke test_non_serializable_handled (record._extra legacy) — added dual-path merge
