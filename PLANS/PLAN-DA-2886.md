...
- [ ] `upload_file(file_path, target: str | UploadSession)`: plain string = legacy fallback; session descriptor = multipart client (direct swap — Protocol widened, never broken)
- [ ] Multipart client: lazy initiate via session token → bounded parallel part PUTs (orchestrator pre-read + pure worker — DA-2882 pattern) → complete; per-part retry/backoff; resume-from-server-truth (DA-2881 pattern); abort-on-failure contract
- [ ] Per-part digest — per DA-2891 verdict: per-part Content-MD5 (RFC 1864), port the DA-2884 machinery as-is (verdict comment id 18196)
- [ ] >5GB node outputs upload successfully; DA-1711 failure semantics preserved (fail/UPLOAD_FAILED at router layer, outside ErrorOutputNode)
- [ ] Graceful degradation BOTH directions: old SDK + new engine (strings), new SDK + old engine (strings)
- [ ] Deprecation: custom `DeprecationWarning` subclass + stacklevel into the node package, deduped per call site; once-per-process `logger.warning`; message names the engine upgrade and removal target (SDK v1.0); standard warnings filtering only (no env flag); ships WITH the multipart client
- [ ] Tests: session handshake, both degradation directions, warning fires once + filterable, multipart happy/failure/resume paths
...
- [ ] **1.1** `UploadSession` pydantic model in `uploads.py` (`kind` literal discriminator, all URL fields + `expires_at`); `request.py` widens `output_upload_url` to `dict[str, str | UploadSession] | None`
...
- [ ] **3.1** `S3PresignedUploader.upload_file(file_path, target: str | UploadSession)`: session → `upload_via_session`; string → legacy single-PUT body (unchanged) + `_warn_legacy_presigned_upload()`. `OutputUploader` Protocol annotation widened identically. `app.py` helper annotations follow.
...
- [ ] **4.1** Handshake/typing: request model accepts mixed str|session dict; rejects malformed session dict (Phase 1 done-when, as tests)
- [ ] **4.2** Degradation both directions: string target → legacy PUT path hit (mock httpx.put) + warning; session target → initiate/complete flow hit
- [ ] **4.3** Warning semantics: once-per-call-site, second-site fires, filterable, operator log once per process
- [ ] **4.4** Multipart happy path: N parts, bounded parallel batches (order-preserved etags), Content-MD5 header per part PUT, explicit Content-Length, complete body carries parts sorted by part_number
- [ ] **4.5** Failure path: part PUT fails after retries → abort_url POSTed, original error re-raised (router fail/UPLOAD_FAILED contract preserved)
- [ ] **4.6** Resume path: part fails post-retry → status_url consulted → confirmed parts adopted as server truth → only remaining re-PUT → complete; status mismatch → abort + original error
...
- [ ] **5.1** `docs/EXTERNAL-AUTHOR-GUIDE.md` + `python/README.md`: document the widened upload contract (what changed for engine operators — nothing for node authors), the deprecation warning + filter recipe, and the session descriptor schema (mirror of the Wire Contract block above)
